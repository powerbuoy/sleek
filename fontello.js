...
class FontelloSassWebpackPlugin {
	...
	convertFontelloCss (code) {
		...
		return css.stringify(obj, {compress: false}).replace(/\.\.\/font\//g, 'fontello/') + sassMixin + sassVars;
	}

	apply (compiler) {
		compiler.hooks.beforeRun.tap('FontelloWebpackPlugin', (compiler) => {
			...
			// Fetch session
			request.post({url: this.url, formData: {config: fontelloConfig}}, (err, res, body) => {
				if (err) {
...
module.exports = FontelloSassWebpackPlugin;
